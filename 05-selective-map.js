...
let selectiveMap = function(array, cb1, cb2) {
    //declare a var to contain the new array
    let newArray = [];

    //iterate through each element of array and set condition
    //if 1st callback is true push 2nd callback result to new array
    //otherwise push the raw element to new array
    /*array.forEach((el) => {
        //cb1(el) ? newArray.push(cb2(el)) : newArray.push(el);
        if (cb1(el)) {
    ...
    Here I tried to use a for loop instead of for.Each
      but it would return an empty array...anyone can tell me why??*/
